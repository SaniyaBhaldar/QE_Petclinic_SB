...
Then('Five text fields labels should be displayed as FirstName, LastName, Address, City, Telephone', async function () {
    ...
    await expect("First Name").to.equal(newOwnerObj.getFirstNameText());
    await expect("Last Name").to.equal(newOwnerObj.getLastNameText());
    await expect("Address").to.equal(newOwnerObj.getAddressText());
    await expect("City").to.equal(newOwnerObj.getCityText());
    await expect("Telephone").to.equal(newOwnerObj.getTelephoneText());
});
...
Then('User should be navigate on Owners page and added owner should be displayed at last', async function () {
    ...
    await expect(appTestData.appTestData.newOwnerData.expectedData).to.equal(globalObj.getLastEntryNewListDetails());
